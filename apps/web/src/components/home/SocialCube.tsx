...
// is often a tofu box or a wildly off-brand shape.
const YouTubeIcon = () => (
...
export default function SocialCube() {
  return (
    <div className="rounded-xl border border-[#e8e0d0] bg-white p-4">
      <p className="mb-4 text-center font-source text-[10px] font-medium uppercase tracking-[0.2em] text-charcoal/40">
        Follow Us
      </p>

      <div className="mx-auto w-[120px] h-[120px] [perspective:600px]">
        ...
        <div className="relative h-[120px] w-[120px] [transform-style:preserve-3d] animate-cubeRotate motion-reduce:animate-none">
          {FACES.map(({ name, Icon, url, bg, face }) => (
            <Link
              key={name}
              href={url}
              target="_blank"
              ...
              aria-label={name}
              className={`absolute flex h-[120px] w-[120px] items-center justify-center rounded-2xl text-white [backface-visibility:hidden] ${bg} ${face}`}
            >
      ...
      {/* Flat row below the cube: the spinning faces are only clickable when
          they happen to be facing front, so these are the reliable targets. */}
      <div className="mt-4 flex justify-center gap-2">
        {FACES.map(({ name, Icon, url, text, hover }) => (
          <Link
            key={name}
            href={url}
            target="_blank"
            rel="noopener noreferrer"
            aria-label={name}
            className={`flex h-8 w-8 items-center justify-center rounded-lg transition-colors duration-200 ${text} ${hover}`}
          >
